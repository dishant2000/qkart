...
const jwtVerify = async (payload, done) => {
  // console.log( "from jwt verify = ", payload)
  try{
    // console.log("mai jwt verify passport js se ,", Date.now(), payload.exp * 1000);
    if (Date.now() > payload.exp * 1000) {
      // console.log("yae chala tha",payload)
    ...
    // console.log("from passportjs ", user, payload.sub);
    if(user){
