...
const generateAuthTokens = async (user) => {
  let accessTokenExpireTime = Math.floor(Date.now() / 1000) + config.jwt.accessExpirationMinutes * 60;

  let accessToken = await generateToken(user._id,accessTokenExpireTime,tokenTypes.ACCESS);

  return {
    access : {
      token : accessToken,
      expires : new Date(accessTokenExpireTime * 1000)
    }
  }
