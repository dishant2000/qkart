...
const addProductToCart = async (user, productId, quantity) => {
  ...
  if(productIndex === -1){
    ...
    if(!product){
      ...
      cart.cartItems.push(cartItemToPush);
      await cart.save();
      return cart;
    }
...
const updateProductInCart = async (user, productId, quantity) => {
  ...
  // for product present in cart or not

  let productIdx = -1;
  for(let i = 0 ; i < cart.cartItems.length ; i++){
    // console.log("id equal or not, ", productId == cart.cartItems[i].product._id);
    if(productId == cart.cartItems[i].product._id){
      productIdx = i;
    }
  }
...
const checkout = async (user) => {
  let cart = await Cart.findOne({email : user.email})
  if(!cart){
    throw new ApiError(httpStatus.NOT_FOUND, "cart is not present");
  }
  if(cart.cartItems.length === 0){
    throw new ApiError(httpStatus.BAD_REQUEST, "cart doesn't have any product");
  }
  let address = user.hasSetNonDefaultAddress();
  if(!address){
  ...
  if(cartTotal > user.walletMoney){
    throw new ApiError(httpStatus.BAD_REQUEST, "not sufficient balance for this purchase");
  }
