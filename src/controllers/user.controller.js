...
const getUser = catchAsync(async (req, res) => {
  // console.log("from user controller,",req,req.params);
  if (req.user.id !== req.params.userId) {
    throw new ApiError(httpStatus.FORBIDDEN);
  } else {
    let userQuery = URL.parse(req.url,true).query;
    let userId = req.params.userId;
    let user;
    // console.log('from user controller', userQuery)
    if(userQuery.q){
      if(userQuery.q === "address"){
        user = await userService.getUserAddressById(userId)
        if (user) {
          // consolce.log(user.address, "from user controller")
